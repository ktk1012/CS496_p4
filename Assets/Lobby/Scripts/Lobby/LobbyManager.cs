...
//		Hashtable users;
        ...
        public override GameObject OnLobbyServerCreateLobbyPlayer(NetworkConnection conn, short playerControllerId)
        {
			...
			if (!currentPlayers.ContainsKey (conn.connectionId)) {
				if (currentPlayers.ContainsValue (0)) {
...
//			if (users.ContainsKey(play
		...
		{
			int idx = currentPlayers [conn.connectionId];
			Debug.Log ("CreateGamePlayer: " + idx.ToString () + conn.ToString ());
			GameObject _temp = (GameObject)GameObject.Instantiate (spawnPrefabs [idx],
